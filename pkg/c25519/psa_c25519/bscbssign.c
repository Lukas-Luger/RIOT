...
// psa_status_t _calculate_r(const uint8_t *R_value, uint8_t *r_value)
// {
//     // R = rG
...
psa_status_t _calculate_r_dash(uint8_t *a_value,
                        uint8_t *b_value,
 ...
 */
psa_status_t _calculate_c_dash(uint8_t *R_dash, uint8_t *c_dash,
                        const uint8_t *msg, size_t msg_len)
{
    ...
    psa_status_t status = psa_hash_compute(PSA_ALG_SHA_512,
                hash_in, hash_in_len, c_dash, HASH_SIZE, &output_len);
    if (status != PSA_SUCCESS || output_len != HASH_SIZE) {
...
psa_status_t psa_bs_cbs_generate_commitment(uint8_t *r0, uint8_t *r1, const uint8_t *output,
                                        size_t size, size_t *length)
{
    if (size < 64) {
        return PSA_ERROR_BUFFER_TOO_SMALL;
    }
    psa_status_t  ret = psa_generate_random(r0, 32);
...
psa_status_t psa_bs_cbs_blind_message(
                    ...
                    const uint8_t *output, size_t output_size, size_t *output_length)
{
    ...
    ret = _calculate_r_dash(a0, b0, pubkey_data, R0_dash, R0);
    if (ret != PSA_SUCCESS) {
        return ret;
    }
...
psa_status_t psa_bs_cbs_sign_message(const psa_key_attributes_t *attributes,
                                    ...
                                    uint8_t *bsignature, size_t bs_size, size_t *bs_length)
{
    (void)attributes;
    uint8_t b_offset;
    psa_status_t ret = psa_generate_random(&b_offset, sizeof(b_offset));
    if (ret != PSA_SUCCESS) {
        return ret;
    }
 ...
 */
psa_status_t _ed25519_pt_equal(struct ed25519_pt *a, struct ed25519_pt *b)
{
    //__print_ed_point(a);
    //__print_ed_point(b);
    ...
    return (error == 2) ? PSA_SUCCESS : PSA_ERROR_INVALID_SIGNATURE;
}
                                         ...
                                         const uint8_t *sig, size_t sig_size)
{
    ...
    // c'_b = HASH(R'_b, msg)
    psa_status_t ret = _calculate_c_dash(R_dash, c_dash, msg, msg_size);
    // puts("c_dash");
...
psa_status_t psa_bs_cbs_unblind_signature(
                    const uint8_t *input, size_t input_length,
